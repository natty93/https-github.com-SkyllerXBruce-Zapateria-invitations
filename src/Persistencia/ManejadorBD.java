...
public class ManejadorBD {
	...
	private ManejadorBD() throws DatabaseException {
		try {
			/*
			 * La instalacion del driver ocurre cuando se carga la clase en un entorno
			 * embebido esto inicia a Derby ya que no esta ejecutandose
			 */
			Class.forName(driver).newInstance();

			// Se puede especificar un usuario y un password
			Properties props = new Properties();
			props.put("user", "Bruce");
			props.put("password", "miguel");

			/*
			 * En la conexion se especifica create=true para crear la base de datos la base
			 * de datos se crea bajo el directorio actual. Para borrarla, basta borrar el
	...
	/**
	 * Este metodo permite obtener un Connection a partir del cual se pueden crear
	 * Statements para ejectuar codigo SQL.
	 *
	 * La primera vez que se invoca este metodo, se checa si ya se creo la instancia
	 * del Manejador de la Base de Datos. Si aun no ha sido creado, se instancia.
	 * Esto solo puede ocurrir una vez, por lo tanto esto representa el patron
	 * singleton.
	 *
	 * @return
	 */
	...
	/**
	 * El metodo finalize se sobre-escribe para cerrar la conexión y apagar la base
	 * de datos cuando el colector de basura recoge al manejador
	 *
	 */
	public static void termina() {
		...
		/*
		 * In embedded mode, an application should shut down Derby. If the application
		 * fails to shut down Derby explicitly, the Derby does not perform a checkpoint
		 * when the JVM shuts down, which means that the next connection will be slower.
		 * Explicitly shutting down Derby with the URL is preferred. This style of
		 * shutdown will always throw an "exception".
		 */
